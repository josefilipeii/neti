      ...
      <main class="flex flex-col items-center justify-center p-6 w-full">
        <HeatWrapper :selectedHeat="heat" :registrationsAvailable="registrations">
          <template  v-if="heat && registrations" #heatTable>
            <HeatTable :heat="heat" :registrations="registrations" :competitionId="selectedCompetition!"/>
          </template>
        </HeatWrapper>
      </main>
    </div>
...
const heats = computed(() => selectedCompetition.value ?
    store.getHeatsByCompetitionId(selectedCompetition.value!!)
    : []);
